...
Seed-данные: **если файл `seed/test-apartment.json` существует** — использовать его как эталонный набор данных для интеграционных тестов. Если файл ещё не создан (до завершения задачи S2-06) — писать unit-тесты с inline-фикстурами (константы прямо в тест-файле). Unit-тесты расчётов (площадь, периметр, объём) не должны зависеть от seed-файла — только интеграционные тесты с реальными данными.
